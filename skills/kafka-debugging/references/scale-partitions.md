...
Call `add_partitions` **without** `confirm`. Nothing is changed. Each item's
result gives the current count, the target, whether messages are keyed, which
consumer groups will rebalance, and the warnings.
...
Call `add_partitions` again with top-level `confirm: true`, adding
`acknowledge_key_ordering: true` to any item whose topic is keyed. The tool
refuses a keyed topic without it.
...
Each topic is one `items` entry carrying its own final target and keyed ordering
acknowledgement; `confirm` is top-level and covers them all. Preview the whole
batch first. Partition changes are not atomic and successful items cannot be
rolled back.
