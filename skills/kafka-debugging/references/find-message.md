...
Addresses go in `items`, one entry per message, so several matches are read in
one call rather than one call each. Keep the returned item errors beside their
indexes; one expired or invalid offset does not invalidate the messages that
were read.
